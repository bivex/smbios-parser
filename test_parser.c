...
static size_t build_smbios(uint8_t *buf, size_t cap) {
    ...
    p[6] = 0x00;
    p[7] = 0xE0; /* BIOSStartingAddressSegment */
    ...
    memcpy(p, bios_s1, strlen(bios_s1) + 1);
    p += strlen(bios_s1) + 1;
    memcpy(p, bios_s2, strlen(bios_s2) + 1);
    p += strlen(bios_s2) + 1;
    memcpy(p, bios_s3, strlen(bios_s3) + 1);
    p += strlen(bios_s3) + 1;
    *p++ = 0; /* end-of-strings terminator */
    ...
    memcpy(p, sys_s1, strlen(sys_s1) + 1);
    p += strlen(sys_s1) + 1;
    memcpy(p, sys_s2, strlen(sys_s2) + 1);
    p += strlen(sys_s2) + 1;
    memcpy(p, sys_s3, strlen(sys_s3) + 1);
    p += strlen(sys_s3) + 1;
    memcpy(p, sys_s4, strlen(sys_s4) + 1);
    p += strlen(sys_s4) + 1;
    *p++ = 0;
