...
static void parse_bios_info(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.bios_info.BIOSReleaseDate = get_string(
            context, context->entry.data.bios_info.BIOSReleaseDate_);
    }
    // 2.4+
...
static void parse_system_info(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.system_info.Manufacturer = get_string(
            context, context->entry.data.system_info.Manufacturer_);
        ...
        context->entry.data.system_info.SerialNumber = get_string(
            context, context->entry.data.system_info.SerialNumber_);
    }
    // 2.1+
...
static void parse_baseboard_info(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.baseboard_info.Manufacturer = get_string(
            context, context->entry.data.baseboard_info.Manufacturer_);
        ...
        context->entry.data.baseboard_info.SerialNumber = get_string(
            context, context->entry.data.baseboard_info.SerialNumber_);
        context->entry.data.baseboard_info.AssetTag = get_string(context, context->entry.data.baseboard_info.AssetTag_);
        context->entry.data.baseboard_info.LocationInChassis = get_string(
            context, context->entry.data.baseboard_info.LocationInChassis_);
    }
}
...
static void parse_system_enclosure(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.system_enclosure.Manufacturer = get_string(
            context, context->entry.data.system_enclosure.Manufacturer_);
        context->entry.data.system_enclosure.Version = get_string(
            context, context->entry.data.system_enclosure.Version_);
        context->entry.data.system_enclosure.SerialNumber = get_string(
            context, context->entry.data.system_enclosure.SerialNumber_);
        context->entry.data.system_enclosure.AssetTag = get_string(
            context, context->entry.data.system_enclosure.AssetTag_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1) {
    ...
    if (context->sversion >= SMBIOS_2_3) {
        ...
        context->ptr += context->entry.data.system_enclosure.ContainedElementCount * context->entry.data.
                system_enclosure.ContainedElementRecordLength;
    }
    // 2.7+
    if (context->sversion >= SMBIOS_2_7) {
        context->entry.data.system_enclosure.SKUNumber_ = read_uint8(context);

        context->entry.data.system_enclosure.SKUNumber = get_string(
            context, context->entry.data.system_enclosure.SKUNumber_);
    }
}
...
static void parse_processor_info(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.processor_info.SocketDesignation = get_string(
            context, context->entry.data.processor_info.SocketDesignation_);
        context->entry.data.processor_info.ProcessorManufacturer = get_string(
            context, context->entry.data.processor_info.ProcessorManufacturer_);
        context->entry.data.processor_info.ProcessorVersion = get_string(
            context, context->entry.data.processor_info.ProcessorVersion_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1) {
    ...
    if (context->sversion >= SMBIOS_2_3) {
        ...
        context->entry.data.processor_info.SerialNumber = get_string(
            context, context->entry.data.processor_info.SerialNumber_);
        context->entry.data.processor_info.AssetTagNumber = get_string(
            context, context->entry.data.processor_info.AssetTagNumber_);
        context->entry.data.processor_info.PartNumber = get_string(
            context, context->entry.data.processor_info.PartNumber_);
    }
    // 2.5+
...
static void parse_system_slot(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.system_slot.SlotDesignation = get_string(
            context, context->entry.data.system_slot.SlotDesignation_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1) {
...
static void parse_memory_device(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_1) {
        ...
        context->entry.data.memory_device.DeviceLocator = get_string(
            context, context->entry.data.memory_device.DeviceLocator_);
        context->entry.data.memory_device.BankLocator = get_string(
            context, context->entry.data.memory_device.BankLocator_);
    }
    // 2.3+
    if (context->sversion >= SMBIOS_2_3) {
        ...
        context->entry.data.memory_device.Manufacturer = get_string(
            context, context->entry.data.memory_device.Manufacturer_);
        context->entry.data.memory_device.SerialNumber = get_string(
            context, context->entry.data.memory_device.SerialNumber_);
        context->entry.data.memory_device.AssetTagNumber = get_string(
            context, context->entry.data.memory_device.AssetTagNumber_);
        context->entry.data.memory_device.PartNumber = get_string(
            context, context->entry.data.memory_device.PartNumber_);
    }
    // 2.6+
    if (context->sversion >= SMBIOS_2_6) {
...
static void parse_port_connector(struct ParserContext *context) {
    ...
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.port_connector.ExternalReferenceDesignator = get_string(
            context, context->entry.data.port_connector.ExternalReferenceDesignator_);
        context->entry.data.port_connector.InternalReferenceDesignator = get_string(
            context, context->entry.data.port_connector.InternalReferenceDesignator_);
    }
}
...
static void parse_management_device(struct ParserContext *context) {
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.management_device.Description = get_string(
            context, context->entry.data.management_device.Description_);
    }
}
...
static void parse_management_device_component(struct ParserContext *context) {
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.management_device_component.Description = get_string(
            context, context->entry.data.management_device_component.Description_);
    }
}
...
static void parse_onboard_devices_extended_info(struct ParserContext *context) {
    if (context->sversion >= SMBIOS_2_0) {
        ...
        context->entry.data.onboard_devices_extended_info.ReferenceDesignation = get_string(
            context, context->entry.data.onboard_devices_extended_info.ReferenceDesignation_);
    }
}
